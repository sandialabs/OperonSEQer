...
def trainSave(X_train, y_train, model, modelname):
    # Names of model
    #X_train = X_train.drop(columns=['KWs','log_KWp','KWABs','log_KWABp','Length1','Length2'])
    # Instantiate the models
    # Train and predict with each model
    model.fit(X_train, y_train)
    filename = str(modelname)+'.p'
    pickle.dump(model, open(filename, 'wb'))



def runModel(model, modelname, X_test, y_test):
    #X_test = X_test.drop(columns=['KWs','log_KWp','KWABs','log_KWABp','Length1','Length2'])
    print('in run model, shape of xtest')
    print(X_test.shape)
    ...
    #tn / (tn + fp)
    results = pd.DataFrame(columns=['accuracy', 'precision','recall','specificity','f1'], index = [modelname])
    preds = {}
    results.loc[modelname, :] = [accuracy, precision, recall, specificity, f1]
    preds[modelname]=predictions
    final=X_test
    final[str(modelname)+'_preds']=preds[modelname]
    results.to_csv(str(modelname)+'_result.csv', sep='\t', index=False)
    final.to_csv(str(modelname)+'_predictions.csv', sep='\t', index=False)
