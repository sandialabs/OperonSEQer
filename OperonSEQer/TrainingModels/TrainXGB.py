...
hparams = {"max_depth": (1,150),"gamma":(0,1),"learning_rate":(0,1),"n_estimators":(10,1000)}
...
# finTarget=svc_bayesopt.max['target']
finDepth=int(svc_bayesopt.max['params']['max_depth'])
finGamma=svc_bayesopt.max['params']['gamma']
finLearn=svc_bayesopt.max['params']['learning_rate']
...
#tn / (tn + fp)
model_name = "XGBoostNoSc"
model_list=["XGBoostNoSc"]
...
final.to_csv('XGBoost_predictionsNoSc.csv', sep='\t', index=False)
