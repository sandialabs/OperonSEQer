...
def estimator(var_smoothing):
    # initialize model
    model = BaggingClassifier(sklearn.naive_bayes.GaussianNB(var_smoothing=var_smoothing), max_samples=0.1, n_estimators=10, bootstrap=False, n_jobs=50)
...
#     model = BaggingClassifier(sklearn.naive_bayes.GaussianNB(var_smoothing=var_smoothing), max_samples=0.1, n_estimators=10, bootstrap=False, n_jobs=10)
#     print('model set')
#     #print(kernel)
#     # set in cross-validation
...
hparams = {"var_smoothing": (1e-9,1)}
#hparams=[{'name':'var_smoothing', 'type':'continuous', 'domain': (1e-9,1)}]
...
svc_bayesopt.maximize(init_points=10, n_iter=100, acq='ucb')
# batch_size=10
# num_cores=10
...
# finTarget=svc_bayesopt.max['target']
finV=svc_bayesopt.max['params']['var_smoothing']
...
model=BaggingClassifier(sklearn.naive_bayes.GaussianNB(var_smoothing=finV), max_samples=0.1, n_estimators=10, bootstrap=False, n_jobs=50)
#model=sklearn.svm.SVC(C=finC, gamma=finGamma, degree=1, random_state=0,kernel='linear')
model.fit(X_train2, y_train)
filename = 'GaussianNB-BayesOp.p'
...
final=X_test2
final['GaussianNB_preds']=preds['GaussianNB']

results.to_csv('GaussianNB_result.csv', sep='\t', index=False)
final.to_csv('GaussianNB_predictions.csv', sep='\t', index=False)
