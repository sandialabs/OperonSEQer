...
#frames = [dfa3, dfb3, dfc3, dfd3, dfe3, dff3]
df = pd.concat(frames)
# df = dfb3
...
#for converting discrete/categorical variables into numerical ones for Bayesian Optimization
#https://mc.ai/bayesian-hyperparameter-optimisation-for-discrete-and-categorical-features/
...
#activationDict = BODict(['identity', 'logistic', 'tanh', 'relu']) #probably relu for interior layers, final layer sigmoid (logistic) or tanh. 
...
def estimator(parameters):
    ...
    # initialize model
    # create the hidden layers as a tuple with length n_layers and n_neurons per layer
    hidden_layer_sizes=(int(parameters[3]),)*int(parameters[2])
    print('we are right before model')
...
#MLP https://www.kaggle.com/plbalmeida/task-1-classifier-bayesian-optimization, https://stackoverflow.com/questions/61502257/optimize-hyperparameters-hidden-layer-size-mlpclassifier-with-skopt


# hparams = [{'activation' : (0, 1),
#         'solver' : (0, 1),
#         'alpha' : (1e-6, 1e-2),
#         'learning_rate' : (0, 1),
#         'max_iter' : (100, 500),
#         'n_hidden_layers' : (1,4),
#         'n_neurons_per_layer' : (200,2000)}]
...
# finSol=svc_bayesopt.X[arg][1]
# finSolC=BOAFunction(solverDict,finsol)
# finAct=svc_bayesopt.X[arg][0]
# finActC=BOAFunction(activationDict,finAct)
finAlpha=svc_bayesopt.X[arg][0]
finNeu=int(svc_bayesopt.X[arg][3])
finLay=int(svc_bayesopt.X[arg][2])
finHLS=(finNeu,)*finLay
finHLS1 = tuple(int(a) for a in finHLS) 
finMax=int(svc_bayesopt.X[arg][1])
# finLR=svc_bayesopt.X[arg][3]
# finLRC=BOAFunction(learning_rateDict,finLR)

# finTarget=svc_bayesopt.max['target']
# finSol=svc_bayesopt.max['params']['solver']
# finSolC=BOAFunction(solverDict,finsol)
# finAct=svc_bayesopt.max['params']['activation']
# finActC=BOAFunction(activationDict,finAct)
# finAlpha=svc_bayesopt.max['params']['alpha']
# finNeu=svc_bayesopt.max['params']['n_neurons_per_layer']
# finLay=svc_bayesopt.max['params']['n_hidden_layers']
# finHLS=(finNeu,)*finLay
# finHLS1 = tuple(int(a) for a in finHLS) 
# finMax=int(svc_bayesopt.max['params']['max_iter'])
# finLR=svc_bayesopt.max['params']['learning_rate']
# finLRC=BOAFunction(learning_rateDict,finLR)
